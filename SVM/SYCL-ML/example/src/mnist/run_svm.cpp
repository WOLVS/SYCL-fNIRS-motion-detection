...
int main(int argc, char** argv) {
  ...
  pca_args.min_nb_vecs = 4;    // Keep at least 64 basis vector
  pca_args.keep_percent = 0.8;  // Keep at least 80% of information
  pca_args.scale_factor = 1E2;  // More accurate but slower PCA

  for (int i = 1; i <= 10; i++) {
    svm_kernel_t ker(0.01 * i);  // Parameter of the RBF kernel
    auto svm_ptr = std::make_unique<ml::svm<svm_kernel_t, label_t>>(C, ker, 2, 0.1, 0.1);
    run_classifier(mnist_path, pca_args, *svm_ptr);
